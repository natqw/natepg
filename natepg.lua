...
-- Check if a player is standing on a plate using a downward raycast
local function isPlayerOnPlate(player)
	if not player.Character then return false end
	local pRoot = player.Character:FindFirstChild("HumanoidRootPart")
	if not pRoot then return false end
	local ray = RaycastParams.new()
	...
	ray.FilterType = Enum.RaycastFilterType.Exclude
	local result = Workspace:Raycast(pRoot.Position, Vector3.new(0, -5, 0), ray)
	if result and result.Instance then
		return result.Instance.Name:lower():find("plate") ~= nil
	end
	return false
...
local function passBomb()
	-- Priority 1: find a player standing on a plate (ideal target)
	-- Priority 2: fall back to nearest alive player if nobody is on a plate
	local plateTarget, plateTargetDist = nil, math.huge
	local nearestFallback, nearestFallbackDist = nil, math.huge
	...
	for _, player in ipairs(Players:GetPlayers()) do
		...
		if isPlayerOnPlate(player) then
			if dist < plateTargetDist then
				plateTargetDist = dist
				plateTarget = player
			end
		else
			if dist < nearestFallbackDist then
				nearestFallbackDist = dist
				nearestFallback = player
			end
		end
	end

	-- Use plate target if found, otherwise fall back to nearest
	local target = plateTarget or nearestFallback
